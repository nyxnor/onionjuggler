...
* 80            then TARGET=127.0.0.1:80
* localhost:80  then TARGET=127.0.0.1:80
* 0             then TARGET=127.0.0.1:VIRTPORT
...
* Localhost with one virtual port:
```sh
...
```
The commands above will have the same effect:
```
HiddenServiceDir /var/lib/tor/services/ssh
HiddenServicePort 22 127.0.0.1:22
```

* Localhost with two virtual ports:
```sh
...
* Remote target with one virtual port
```sh
...
* Remote target with two virtual ports:
```sh
bash onion-service.sh on tcp ssh 22 192.168.0.10:22
bash onion-service.sh on tcp ssh 22 [IPV6]:22
```
...
```sh
bash onion-service.sh on unix ssh 22
```
```
HiddenServiceDir /var/lib/tor/services/ssh
...
```sh
bash onion-service.sh on unix ssh 22 80
```
```
HiddenServiceDir /var/lib/tor/services/ssh
HiddenServicePort 22 /var/run/tor-hs-ssh-22.sock
...
```sh
bash onion-service.sh auth server on all-services alice,bob
```

Example:
```sh
bash onion-service.sh auth server on test,xmpp alice,bob
```
Effect:
```
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
```sh
bash onion-service.sh auth server off ssh,xmpp,nextcloud alice,bob
```

Remove authorization of all services and one or a list of clients:
```sh
bash onion-service.sh auth server off all-services alice,bob
```

Remove authorization of all services and all clients:
```sh
bash onion-service.sh auth server off all-services all-clients
```
...
```
<onion-address-without-.onion>:descriptor:<client-private-key>
```
...
The operator is named fritz and his site is about culinaire (create unique names, can even use the onion address in the name of the file). To add your key to authenticate with tor (daemon):
```sh
...
```sh
bash onion-service.sh auth client list
```
Effect:
```
...
```sh
bash onion-service.sh credentials ssh,xmpp,nextcloud
```

View credentials of all services:
```sh
bash onion-service.sh credentials all-services
```
